...
```{r, echo = FALSE}
rmarkdown::metadata$filters
```

```{r, echo = FALSE}
estimate <- round(as.integer(rmarkdown::metadata$filters['wordcount']) / 200, 0)
```
...
The total reading time for this case study is calculated by finding a word count using a [quarto extension](https://github.com/christopherkenny/wordcount/tree/main) and estimating time based on an average adult reading speed of 200 words per minute.
